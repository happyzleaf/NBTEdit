						...
						if (te != null && NBTEdit.proxy.checkPermission(player)) {
							try {
								...
								if (te.hasWorldObj() && te.getWorld() instanceof WorldServer) {
									((WorldServer) te.getWorld()).getPlayerManager().markBlockForUpdate(packet.pos);// Broadcast changes.
								}
								...
								NBTEdit.proxy.sendMessage(player, "Save Failed - Invalid NBT format for Tile Entity", EnumChatFormatting.RED);
							...
							NBTEdit.proxy.sendMessage(player, "Save Failed - There is no TileEntity at " + packet.pos.getX() + ", " + packet.pos.getY() + ", " + packet.pos.getZ(), EnumChatFormatting.RED);
						}
					}
				});
			} else {
